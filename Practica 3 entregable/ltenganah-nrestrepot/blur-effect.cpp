...
int main(int argc, char *argv[]) {
  ...
  // imwrite(  argv[2], result );
  		...
  		if ( myId == MANAGER )	printf("%f", time_taken);
...
void doManager( Mat img, int n, int noProcs, string image_name) {
  ...
  for ( i=1; i<noProcs; i++ ) {
    ...
    width_slice = end-begin;
    if(DEBUGGER == 1)printf("Receiving from process %d. width_slice : %d\n", i, width_slice);
  ...
  if(DEBUGGER == 1) printf("Writing final image!\n");
...
//  imwrite( out_string + "_temporal.jpg", image_roi );
  ...
  //   }
  // }
	if(DEBUGGER == 1)printf("Sended %d from process %d %d \n  ", n, begin, end);
}
/*
